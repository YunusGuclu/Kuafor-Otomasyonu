...
namespace Kuaför
{
    public partial class Ödeme_Yönetimi : Form
    {
        public Ödeme_Yönetimi()
        {
            InitializeComponent();
        }
        ...
        private void Ödeme_Yönetimi_Load(object sender, EventArgs e)
        {           
            listele();
        ...
        private void btn_Ekle_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    baglanti.Open();
                    DateTime secilenRandevuTarihi = date_İşlem_Tarihi.Value.Date;
                    ...
                    OleDbCommand komut = new OleDbCommand("insert into Ödemeler([Ad Soyad],Telefon,[Yapılan İşlem],[İşlem Tarihi],Kuaför,Ücret)values('" + txt_Ad_Soyad.Text + "','" + masked_Telefon.Text + "','" + txt_Yapılan_İşlem.Text + "','" + tarihString + "','" + combo_Kuaför.Text + "','" + txt_Ücret.Text + "')", baglanti);
                ...
                if (btn_Ekle.Text == "Güncelle")
                {
                    ...
                    // Tarih bilgisini belirli bir formatta string'e dönüştürüyorum
                    string tarihString = secilenRandevuTarihi.Date.ToString("dd.MM.yyyy");
                    OleDbCommand komut2 = new OleDbCommand("UPDATE Ödemeler SET [Ad Soyad]='" + txt_Ad_Soyad.Text + "',[Yapılan İşlem]='" + txt_Yapılan_İşlem.Text + "', [İşlem Tarihi]='" + tarihString + "',Kuaför='" + combo_Kuaför.Text + "',Ücret='" + txt_Ücret.Text + "' WHERE Telefon='" + masked_Telefon.Text + "'", baglanti);
        ...
        private decimal ToplamGeliriHesapla(string tarihFiltresi)
        {
            decimal toplamGelir = 0;

            using (OleDbConnection baglanti = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=Ödemeler.mdb"))
            {
                baglanti.Open();
                string sorgu = "SELECT Ücret FROM Ödemeler WHERE " + tarihFiltresi;
                ...
                using (OleDbCommand komut = new OleDbCommand(sorgu, baglanti))
                {
                    using (OleDbDataReader okuyucu = komut.ExecuteReader())
                    {
                        while (okuyucu.Read())
                        {
                            decimal miktar = Convert.ToDecimal(okuyucu["Ücret"]);
                            toplamGelir += miktar;
                        }
                    }
                }
        ...
        private void GeliriHesaplaVeGoster(string tarihFiltresi)
        {
            decimal toplamGelir = ToplamGeliriHesapla(tarihFiltresi);
            MessageBox.Show($"Seçilen dönem için Toplam Gelir: {toplamGelir.ToString("C")}", "Gelir Hesaplama");
        }

        private void btn_Aylık_Kazanc_Click(object sender, EventArgs e)
        {
            DateTime secilenTarih = date_İşlem_Tarihi.Value;
            string tarihFiltresi = "Month([İşlem Tarihi]) = " + secilenTarih.Month + " AND Year([İşlem Tarihi]) = " + secilenTarih.Year;
            GeliriHesaplaVeGoster(tarihFiltresi);
        }

        private void btn_Yıllık_Kazanc_Click(object sender, EventArgs e)
        {
            DateTime secilenTarih = date_İşlem_Tarihi.Value;
            string tarihFiltresi = "Year([İşlem Tarihi]) = " + secilenTarih.Year;
            GeliriHesaplaVeGoster(tarihFiltresi);
        }
